...
> ⚠️ **华为 / 荣耀 / 小米 / OPPO / vivo 用户请先读[后台放行指南](docs/oem-background-whitelist.md)。**
> 这类系统默认把「从最近任务划掉」当成**强行停止**，而强行停止会让系统取消应用的全部闹钟 ——
> 不做放行的话，划掉之后闹钟不会响，而且没有任何提示。
...
- **国产 ROM 的后台管控 —— 这是本应用在真机上最常见的失效原因。** 华为 / 小米 / OPPO / vivo
  默认把「从最近任务划掉」当成**强行停止**，而**强行停止会让系统取消应用的全部闹钟**。
  表现就是划掉之后「闹钟根本不响，也没有声音和通知」。**这一步无法用代码绕过**，必须手动放行。
  👉 **[后台放行指南（含各品牌具体路径）](docs/oem-background-whitelist.md)**
...
在真机（华为 / HarmonyOS 4）上：

| 场景 | 结果 |
|---|---|
| 闹钟到点时应用在**后台** | ✅ 完整响铃，答题界面正常弹出 |
| 闹钟到点前把应用**从最近任务划掉** | ❌ 到点完全无反应；应用的响铃诊断记录里连「收到闹钟广播」都没有 |

> 真机第二行就是「强行停止」的实际表现 —— 也是上面那份[后台放行指南](docs/oem-background-whitelist.md)存在的原因。
> 注意模拟器**测不出**这一条：模拟器的 `am kill` 只是普通进程回收，闹钟会照常触发。
