    ...
    Column(
        ...
        Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant)) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Limitation(
                    "国产 ROM 的后台管控（华为 / 小米 / OPPO / vivo 必看）",
                    "这类系统在「从最近任务划掉」或长时间息屏后会直接强行停止应用，" +
                        "而应用一旦被强行停止，系统就会取消它的全部闹钟 —— 表现就是「闹钟根本不响」。\n\n" +
                        "请手动放行：设置 → 应用 → 应用启动管理 → 任务闹钟 → 改为「手动管理」，" +
                        "把自启动 / 关联启动 / 后台活动三项全部打开；再到电池设置里把它加入「受保护应用」" +
                        "或把省电策略设为「无限制」。华为机型还需要在手机管家里允许自启动。",
                )
                Limitation(
                    "亮屏时答题界面不会自动弹出",
                    "系统只在锁屏或息屏时才会自动拉起全屏通知里的界面。如果你正在用手机（亮屏），" +
                        "闹钟照样会响、也会弹悬浮通知，但答题界面需要点一下通知才出现。" +
                        "这是 Android 对后台启动界面的限制，不是应用的问题。",
                )
                Limitation(
                    "铃声片段只在本应用内有效",
